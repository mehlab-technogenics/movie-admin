...
function dateTimeAggregationQuery(matchWherePattern, fieldName, targetAlias) {
    const fieldPath = `${targetAlias}.${fieldName}`;
    return `${matchWherePattern}
        RETURN ${stringify_object_1.stringifyObject({
        min: create_datetime_element_1.wrapApocConvertDate(`min(${fieldPath})`),
        max: create_datetime_element_1.wrapApocConvertDate(`max(${fieldPath})`),
    })}`;
}
